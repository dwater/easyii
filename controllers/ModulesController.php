        ...
        if ($formModel->load(Yii::$app->request->post())){
            if(Yii::$app->request->isAjax){
                ...
                foreach(glob($newModuleFolder . DIRECTORY_SEPARATOR . 'models' . DIRECTORY_SEPARATOR . '*.php') as $modelFile){
                    ...
                    try {
                        $oldRawTableName =  Yii::$app->db->getSchema()->getRawTableName($oldTableName);
                        $newRawTableName =  Yii::$app->db->getSchema()->getRawTableName($newTableName);
                        //Drop new table if exists
                        Yii::$app->db->createCommand("DROP TABLE IF EXISTS `$newRawTableName`;")->execute();

                        //Copy new table
                        Yii::$app->db->createCommand("CREATE TABLE `$newRawTableName` LIKE `$oldRawTableName`;")->execute();
