...
class Feedback extends \yii\easyii\components\API
{
    ...
    public function api_form($options = [])
    {
        ...
        echo $form->field($model, 'name')->textInput(['placeholder'=>'名字'])->label('');
        echo $form->field($model, 'email')->input('email')->textInput(['placeholder'=>'Email'])->label('');

        if($settings['enablePhone']) echo $form->field($model, 'phone')->textInput(['placeholder'=>'电话号码'])->label('');
        if($settings['enableTitle']) echo $form->field($model, 'title')->textInput(['placeholder'=>'主题'])->label('');

        echo $form->field($model, 'text')->textarea(['placeholder'=>'内容'])->label('');

        if($settings['enableCaptcha']) echo $form->field($model, 'reCaptcha')->widget(ReCaptcha::className());

        echo Html::submitButton(Yii::t('easyii', 'Submit'), ['class' => 'btn btn-submit']);
        ActiveForm::end();
