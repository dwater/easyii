...
class Image
{
    ...
    static function thumb($filename, $width = null, $height = null, $crop = true)
    {
        ...
        if($filename && file_exists(($filename = Yii::getAlias('@uploadsRoot') . $filename)))
        {
            ...
            $thumbFile = Yii::getAlias('@uploadsRoot') . DIRECTORY_SEPARATOR . Upload::$UPLOADS_DIR . DIRECTORY_SEPARATOR . 'thumbs' . DIRECTORY_SEPARATOR . $thumbName;
            $thumbWebFile = '/' . Upload::$UPLOADS_DIR . '/thumbs/' . $thumbName;
            if(file_exists($thumbFile)){
                //return $thumbWebFile;
                return Yii::getAlias('@uploadsUrl').$thumbWebFile;
            }
